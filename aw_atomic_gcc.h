...
#define _aw_impl_cas(ptr, exp, des, succ, fail) \
    __atomic_compare_exchange_n(ptr, exp, des, 0, succ, fail)

#define _aw_impl_fetch_add(ptr, val, order) \
    __atomic_fetch_add(ptr, val, order)

#define _aw_impl_fetch_sub(ptr, val, order) \
    __atomic_fetch_sub(ptr, val, order)

#define _aw_impl_fetch_and(ptr, val, order) \
    __atomic_fetch_and(ptr, val, order)

#define _aw_impl_fetch_or(ptr, val, order) \
    __atomic_fetch_or(ptr, val, order)

#define _aw_impl_fetch_xor(ptr, val, order) \
